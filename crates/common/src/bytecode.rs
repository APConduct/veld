...
impl std::hash::Hash for Instruction {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        match self {
            ...
            Instruction::Closure(index, upvalue_count) => {
                state.write_u8(39);
                index.hash(state);
                upvalue_count.hash(state);
            }
            Instruction::CloseUpvalues(index) => {
                state.write_u8(40);
                index.hash(state);
            }
            Instruction::NewStruct(index) => {
                state.write_u8(41);
                index.hash(state);
            }
            ...
            Instruction::NewEnum(index, size) => {
                state.write_u8(48);
                index.hash(state);
                size.hash(state);
            }
            ...
            Instruction::MatchPattern(index) => {
                state.write_u8(50);
                index.hash(state);
            }
            ...
            Instruction::TypeCheck(index) => {
                state.write_u8(53);
                index.hash(state);
            }
            Instruction::TypeCast(index) => {
                state.write_u8(54);
                index.hash(state);
            }
            Instruction::Import(index) => {
                state.write_u8(55);
                index.hash(state);
            }
            Instruction::Export(index) => {
                state.write_u8(56);
                index.hash(state);
            }
            Instruction::Print => {
...
impl std::hash::Hash for Chunk {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        ...
        self.line_numbers.hash(state);
        self.source_file.hash(state);
        self.name.hash(state);
        self.local_count.hash(state);
        self.parameter_count.hash(state);
        self.max_stack_depth.hash(state);
        self.upvalues.hash(state);
        self.labels.hash(state);
        self.debug_symbols.hash(state);
    }
}
